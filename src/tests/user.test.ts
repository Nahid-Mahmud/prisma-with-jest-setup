...
describe("User Routes", () => {
  ...
  describe("POST /api/v1/users", () => {
    ...
    it("should not allow duplicate user", async () => {
      ...
      const response = await request(app).post("/api/v1/users").send(newUser).expect(400);
  ...
  describe("GET /api/v1/users/:id", () => {
    it("should get a user by ID", async () => {
      ...
      const response = await request(app).get(`/api/v1/users/${createdUser.id}`).expect(200);
  ...
  describe("PATCH /api/v1/users/:id", () => {
    it("should update a user's name", async () => {
      ...
      const response = await request(app).patch(`/api/v1/users/${user.id}`).send(updates).expect(200);
  ...
  describe("DELETE /api/v1/users/:id", () => {
    it("should delete a user", async () => {
      ...
      const response = await request(app).delete(`/api/v1/users/${user.id}`).expect(200);
